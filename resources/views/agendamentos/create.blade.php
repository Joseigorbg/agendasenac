                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center justify-between">
                        <label class="flex items-center text-gray-900 shadow-sm">
                            <input type="checkbox" name="equipamentos[{{ $equipamento }}][selected]" value="1" class="mr-2" @change="selectedEquipments.includes('{{ $equipamento }}') ? selectedEquipments = selectedEquipments.filter(e => e !== '{{ $equipamento }}') : selectedEquipments.push('{{ $equipamento }}')">
                            {{ ucfirst(str_replace('_', ' ', $equipamento)) }}
                        </label>
                        <!-- Campo de quantidade -->
                        <input type="number" name="equipamentos[{{ $equipamento }}][quantity]" min="1" value="1" class="form-input mt-1 w-20 border border-gray-300 rounded-md shadow-sm focus:ring-senacOrange focus:border-senacOrange" x-show="selectedEquipments.includes('{{ $equipamento }}')" placeholder="Qtd">
                    </div>
                    @endforeach
                </div>
